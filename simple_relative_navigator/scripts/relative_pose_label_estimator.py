...
from relative_navigator_msgs.msg import SimpleRelPoseLabel
...
class RelativePoseLabelEstimator:
    def __init__(self) -> None:
        ...
        self._model: SimpleAbstRelPosNet = SimpleAbstRelPosNet().to(self._device)
        ...
        self._abstract_relative_pose_pub: rospy.Publisher = rospy.Publisher("/simple_relative_pose_label", SimpleRelPoseLabel, queue_size=1)
    ...
    def _predict_abstract_relative_pose(self) -> SimpleRelPoseLabel:
        models_output: torch.Tensor = self._model(self._observed_image.to(self._device),self._reference_image.to(self._device))
        relative_pose_label: int = self._models_output_to_label_list(models_output)
        simple_rel_pose_label_msg = SimpleRelPoseLabel()
        simple_rel_pose_label_msg.label = relative_pose_label
        simple_rel_pose_label_msg.header.stamp = rospy.Time.now()

        return simple_rel_pose_label_msg

    def _models_output_to_label_list(self, models_output: torch.Tensor) -> int:
    ...
    def process(self) -> None:
        ...
        while not rospy.is_shutdown():
            ...
            if self._reaching_target_pose_flag == None or self._reaching_target_pose_flag:
                ...
                reaching_goal_flag: bool = abst_rel_pose_msg.label == 0
