...
class DatasetGeneratorForAbstRelPosNet(DatasetGenerator):
    ...
    def _generate_data_from_reference_points(self, reference_point1: ReferencePoint,
        ...
        self._save_data(images,
                torch.tensor([direction_label, orientation_label], dtype=torch.float32),
    ...
    def _to_direction_label(self, relative_pose: List[float]) -> List[float]:
        direction_label: List[float] = [0] * (self._config._bin_num+1)
    ...
    def _to_orientation_label(self, relative_pose: List[float]) -> List[float]:
        orientation_label: List[float] = [0] * self._config._bin_num
    ...
    def _is_appropriate_pair(self, reference_point1: ReferencePoint, reference_point2: ReferencePoint) -> bool:
        ...
        if abs(direction_yaw) > self._config.yaw_gap_th: return False

        orientation_yaw: float = relative_pose[2]
        if abs(orientation_yaw) > self._config.yaw_gap_th: return False
    ...
    def _allocate_yaw_to_bin(self, yaw: float) -> int:
        ...
        if abs(yaw) > self._config.yaw_gap_th: return -1

        # binの割当に使うboarderをbin no 0 の最小値で初期化
        boarder_yaw: float = radians((self._config._bin_num // 2 - 0.5) * self._config.bin_step_degree)
        for bin_no in range(self._config._bin_num):
            if bin_no < self._config._bin_num-1 and yaw < boarder_yaw or \
                    bin_no == self._config._bin_num-1 and yaw <= boarder_yaw:
                return bin_no 
