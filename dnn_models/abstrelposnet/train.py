...
def main():
    ...
    num_data: int = min(args.num_data, len(dataset))

    dataset, _ = random_split(dataset, [num_data, len(dataset) - num_data],
            generator=torch.Generator().manual_seed(args.seed))

    train_len = int(num_data * args.train_ratio / 10)
    train_dataset, valid_dataset = random_split(
                dataset, [train_len, num_data -train_len], generator=torch.Generator().manual_seed(args.seed)
            )
    ...
    criterion_for_direction = FocalLoss(fl_gamma=0,
                                        samples_per_class=direction_label_counts.tolist(),
                                        class_balanced=True, beta=0)
    criterion_for_orientation = FocalLoss(fl_gamma=0,
    ...
    scheduler = optim.lr_scheduler.CyclicLR(
                optimizer, base_lr=args.lr_min, max_lr=args.lr_max, step_size_up=step_size_up, mode="triangular", cycle_momentum=False)
    ...
    for epoch in range(args.num_epochs):
        ...
        with tqdm(total=len(train_loader), ncols=100) as progbar:
            ...
            for data in train_loader:
                ...
                epoch_train_sum_loss += train_sum_loss
                epoch_train_direction_loss += direction_loss
                epoch_train_orientation_loss += orientation_loss
        ...
        with torch.no_grad():
            ...
            for data in valid_loader:
                ...
                # print(f"onehot_output_direction: {onehot_output_direction}")
                # print(f"onehot_output_orientation: {onehot_output_orientation}")
                # print(f"direction_labe: {direction_label}")
                # print(f"orientation_labe: {orientation_label}")
                ...
                # direction_judge_tensor = (onehot_output_direction == direction_label.to(device)) == \
                #                          (onehot_output_direction == 1)
                # print(f"judge_tensor: {direction_judge_tensor}")
                ...
                # orientation_judge_tensor = (onehot_output_orientation == orientation_label.to(device)) == \
                #                          (onehot_output_orientation == 1)
                ...
                # train_sum_loss = direction_loss + orientation_loss
                epoch_valid_sum_loss += valid_sum_loss
                epoch_valid_direction_loss += direction_loss
                epoch_valid_orientation_loss += orientation_loss
            ...
            # print(f"direction_label_counts: {direction_label_counts_valid}")
            # print(f"orientation_label_counts: {orientation_label_counts_valid}")
            # print(f"direction_label_correct_count: {direction_label_correct_count}")
            # print(f"orientation_label_correct_count: {orientation_label_correct_count}")
        ...
        if best_loss > epoch_valid_sum_loss:
