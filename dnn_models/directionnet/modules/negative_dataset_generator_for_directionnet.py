...
class ConfigForNegativeGenerator(Config):
    # dist_gap_th: float = 5.0
    # dist_labelling_th: float = 1.5
    total_travel_dist_gap_th: float = 20.0

    # 以下のパラメータは fov_degree/bin_step_degree>=3 になるように設定すること
    fov_degree: int = 78
    bin_step_degree: int = 25

class NegativeDatasetGeneratorForDirectionNet(DatasetGenerator):
    # param determin by user-defined param
    _bin_num: int
    # _yaw_gap_th: float

    def __init__(self, config: ConfigForNegativeGenerator, bag: Bag, bag_id: int) -> None:
        super().__init__(config, bag, bag_id)
        self._config:ConfigForNegativeGenerator = config
    ...
    def __call__(self) -> None:
        ...
        for reference_point1 in self._reference_points:
            ...
            nearest_id = self._search_nearest_id(reference_point1)
            if nearest_id == len(self._reference_points) - 1: break
            # if not self._is_negative_pair(reference_point1, self._reference_points[nearest_id]):
            #     continue

            while True:
                # point2_id: int = random.randint(reference_point1.point_index, furthest_id)
                point2_id: int = random.randint(nearest_id, len(self._reference_points)-1)
                ...
                if self._is_negative_pair(reference_point1, reference_point2): break
                # if reference_point1.point_index == len(self._reference_points)-1: break

            self._generate_data_from_reference_points(reference_point1, reference_point2)

    def _search_nearest_id(self, reference_point: ReferencePoint) -> int:
    ...
    def _generate_data_from_reference_points(self, reference_point1: ReferencePoint,
        ...
        if not self._is_negative_pair(reference_point1, reference_point2): return
    ...
    def _is_negative_pair(self, reference_point1: ReferencePoint, reference_point2: ReferencePoint) -> bool:
        # この場合atan2がバグるので先に処理
        if reference_point1.point_index == reference_point2.point_index: return False

        if abs(reference_point1.total_travel_distance - reference_point2.total_travel_distance) > \
            self._config.total_travel_dist_gap_th: return True
