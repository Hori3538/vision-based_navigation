...
class DatasetForDirectionNet(Dataset):
    ...
    def count_data_for_each_label(dataset) -> Tuple[torch.Tensor, ...]:
        ...
        direction_label_counts: torch.Tensor = torch.tensor([0] * (bin_num+2), dtype=torch.float)
...
def test() -> None:
    ...
    dataset = DatasetForDirectionNet(args.dataset_dir, use_transform=False)
    # データ確認用loader
    ...
    direction_label_counts, _ = DatasetForDirectionNet.count_data_for_each_label(dataset)
