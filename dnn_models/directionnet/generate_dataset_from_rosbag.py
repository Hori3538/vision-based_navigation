...
from dataset_generator_for_directionnet import (ConfigForDirectionNet,
        DatasetGeneratorForDirectionNet)

def generate_dataset(config, bagfile_path, bag_id) -> None:
    bag = Bag(bagfile_path)
    DatasetGeneratorForDirectionNet(config, bag, bag_id=bag_id)()
    
def main() -> None:
    config = parse_args(ConfigForDirectionNet)
