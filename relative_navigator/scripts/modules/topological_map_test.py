...
class TopologicalMapTest:
    def __init__(self) -> None:
        ...
        self._param = Param(
            ...
            cast(str, rospy.get_param("~localize_error_output_dir")),
    ...
    def _write_localize_error_from_bag(self, bag: Bag, bag_id: int, output_path: str) -> None:
        ...
        with open(output_path, 'w') as f:
            writer = csv.writer(f)
            for topic, msg, ros_time in bag.read_messages(
                    topics=[self._param.pose_topic_name, "/graph_localizer/nearest_node_id"]):

                if topic == self._param.pose_topic_name:
                    gt_pose = msg_to_pose(msg, self._param.pose_topic_type)
                if topic == "/graph_localizer/nearest_node_id":
                    nearest_node = msg.data

                if gt_pose is None or nearest_node is None: continue

                sec, nsec = ros_time.secs, ros_time.nsecs
                time: float = sec + nsec/10e9
                if initial_time is None: initial_time = time
                elapsed_time = time - initial_time
                
                predict_x, predict_y = self._get_coord_of_node(self._graph, nearest_node)
                dist = math.hypot(gt_pose.position.x - predict_x, gt_pose.position.y - predict_y)
                # print(f"dist: {dist}")
                # print(f"elapsed_time: {elapsed_time}")
                writer.writerow([elapsed_time, dist])

                gt_pose = None
                nearest_node = None
        rospy.loginfo(f"bag id: {bag_id} is finished")

    def _write_localize_error_from_bag_dir(self, bag_dir: str) -> None:
        parent_dir = os.path.abspath(os.path.join(bag_dir, os.pardir))
        ouput_dir_name: str = os.path.basename(parent_dir) + "_localize_error"
        output_path: str = os.path.join(self._param.localize_error_output_dir, ouput_dir_name)
        ...
        for i, bagfile_path in enumerate(iglob(os.path.join(bag_dir, "*.bag"))):
            ...
            bagfile_name_without_ext = os.path.splitext(os.path.basename(bagfile_path))[0]
            ouputfile_path = os.path.join(output_path, bagfile_name_without_ext) + ".csv"
            print(f"ouputfile_path: {ouputfile_path}")

            self._write_localize_error_from_bag(bag, i, ouputfile_path)
    ...
    def _search_nearest_node(self, coord: Tuple[float, float], graph: Union[nx.DiGraph, nx.Graph]) -> str:
        ...
        for tgt_node, tgt_pose in dict(graph.nodes.data('pose')).items():
            ...
            if dist < min_dist:
                min_dist = dist
                nearest_node = cast(str, tgt_node)
    ...
    def _calc_progress_rate(self, graph: Union[nx.Graph, nx.DiGraph], initial_coord: Tuple[float, float], goal_coord: Tuple[float, float], current_coord: Tuple[float, float]) -> float:

        initial_node: str = self._search_nearest_node(initial_coord, graph)
        goal_node: str = self._search_nearest_node(goal_coord, graph)
        current_node: str = self._search_nearest_node(current_coord, graph)

        path_initial_to_goal: List[str] = self._calc_shortest_path(graph, initial_node, goal_node)
        path_current_to_goal: List[str] = self._calc_shortest_path(graph, current_node, goal_node)
        dist_initial_to_goal: float = self._calc_dist_of_path(graph, path_initial_to_goal)
        dist_current_to_goal: float = self._calc_dist_of_path(graph, path_current_to_goal)

        progress_rate: float = max(1 - dist_current_to_goal/dist_initial_to_goal, 0)

        return progress_rate

    def _calc_max_progress_rate_from_bag(self, bag: Bag, gt_graph: nx.Graph, goal_coord: Tuple[float, float]) -> float:

        pose: Optional[Pose] = None

        initial_coord: Optional[Tuple[float, float]] = None

        max_progress_rate: float = 0
        ...
        for topic, msg, _ in bag.read_messages(
            ...
            current_coord: Tuple[float, float] = pose.position.x, pose.position.y
            if initial_coord is None: initial_coord = current_coord
            progress_rate: float = self._calc_progress_rate(gt_graph, initial_coord, goal_coord,
                    current_coord)
            if progress_rate > max_progress_rate: max_progress_rate = progress_rate

            pose = None

        return max_progress_rate

    def _calc_max_progress_rate_from_bag_dir(self, bag_dir: str, gt_graph: nx.Graph, goal_coord: Tuple[float, float]) -> None:
        for i, bagfile_path in enumerate(iglob(os.path.join(bag_dir, "*.bag"))):
            rospy.loginfo(f"bagfile_path {bagfile_path} start")
            bag: Bag = Bag(bagfile_path)
    ...
    def process(self) -> None:
        ...
        # start_node, goal_node = "0_105", "0_12" # 4_re_re
        # start_node, goal_node = "3_110", "2_140" # 5 re
        # start_node, goal_node = "2_90", "2_145" # 6

        # without backlight
        # start_node, goal_node = "0_0", "0_80" # 

        goal_node_coord: Tuple[float, float] = self._get_coord_of_node(self._graph, goal_node)
